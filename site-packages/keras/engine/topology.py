...
class Layer(object):
    ...
    def __init__(self, **kwargs):
        ...
        if 'batch_input_shape' in kwargs or 'input_shape' in kwargs:
            ...
            batch_input_shape = (
                tuple(kwargs['batch_input_shape'])
                if 'batch_input_shape' in kwargs
                else (None,) + tuple(kwargs['input_shape'])
            )
    ...
    def non_trainable_weights(self):
        ...
        return (
            self._non_trainable_weights
            if trainable
            else self._trainable_weights + self._non_trainable_weights
        )
    ...
    def create_input_layer(self, batch_input_shape,
        ...
        if not name:
            prefix = f'{self.__class__.__name__.lower()}_input_'
    ...
    def assert_input_compatibility(self, input):
        ...
        if len(self.input_spec) > 1 and len(inputs) != len(self.input_spec):
            raise ValueError(
                f'Layer {self.name} expects {len(self.input_spec)} inputs, but it received {len(inputs)} input tensors. Input received: {str(input)}'
            )
        ...
        for input_index, (x, spec) in enumerate(zip(inputs, self.input_spec)):
            ...
            if spec.ndim is not None:
                if isinstance(spec.ndim, str):
                    ...
                    if K.ndim(x) < ndim:
                        raise ValueError(
                            f'Input {str(input_index)} is incompatible with layer {self.name}: expected ndim >= {ndim}, found ndim={str(K.ndim(x))}'
                        )

                elif K.ndim(x) != spec.ndim:
                    raise ValueError(
                        f'Input {str(input_index)} is incompatible with layer {self.name}: expected ndim={str(spec.ndim)}, found ndim={str(K.ndim(x))}'
                    )

            if spec.dtype is not None and K.dtype(x) != spec.dtype:
                raise ValueError(
                    f'Input {str(input_index)} is incompatible with layer {self.name}: expected dtype={str(spec.dtype)}, found dtype={str(K.dtype(x))}'
                )
            ...
            if spec.shape is not None:
                ...
                for spec_dim, dim in zip(spec.shape, x_shape):
                    if spec_dim is not None and spec_dim != dim:
                        raise ValueError(
                            f'Input {str(input_index)} is incompatible with layer {self.name}: expected shape={str(spec.shape)}, found shape={str(x_shape)}'
                        )
    ...
    def add_inbound_node(self, inbound_layers,
        ...
        if not node_indices:
            ...
            assert len(node_indices) == len(inbound_layers)
        tensor_indices = (
            to_list(tensor_indices)
            if tensor_indices
            else [0 for _ in range(len(inbound_layers))]
        )
        ...
        if not hasattr(self, 'supports_masking') or not self.supports_masking:
            if input_mask is not None:
                if not isinstance(input_mask, list):
                    raise ValueError(
                        f'Layer {self.name} does not support masking, but was passed an input_mask: {str(input_mask)}'
                    )
    ...
    def _get_node_attribute_at_index(self, node_index, attr, attr_name):
        ...
        if len(self.inbound_nodes) <= node_index:
            raise ValueError(
                f'Asked to get {attr_name} at node {str(node_index)}, but the layer has only {len(self.inbound_nodes)} inbound nodes.'
            )
    ...
    def input(self):
        ...
        if len(self.inbound_nodes) > 1:
            raise AttributeError(
                (
                    f'Layer {self.name}' + ' has multiple inbound nodes, '
            ...
            raise AttributeError(
                f'Layer {self.name} is not connected, no input to return.'
            )
    ...
    def output(self):
        ...
        if len(self.inbound_nodes) == 0:
            raise AttributeError(f'Layer {self.name} has no inbound nodes.')
        if len(self.inbound_nodes) > 1:
            raise AttributeError(
                (
                    f'Layer {self.name}' + ' has multiple inbound nodes, '
    ...
    def input_shape(self):
        ...
        all_input_shapes = {str(node.input_shapes) for node in self.inbound_nodes}
    ...
    def output_shape(self):
        ...
        all_output_shapes = {str(node.output_shapes) for node in self.inbound_nodes}
    ...
    def add_loss(self, losses, inputs=None):
        ...
        inputs_hash = object_list_uid(inputs) if inputs is not None else None
    ...
    def set_weights(self, weights):
        ...
        for pv, p, w in zip(param_values, params, weights):
            if pv.shape != w.shape:
                raise ValueError(
                    f'Layer weight shape {str(pv.shape)} not compatible with provided weight shape {str(w.shape)}'
                )
...
class InputLayer(Layer):
    ...
    def __init__(self, input_shape=None, batch_input_shape=None,
        ...
        if not input_dtype:
            input_dtype = K.floatx() if input_tensor is None else K.dtype(input_tensor)
        self.batch_input_shape = batch_input_shape
        self.input_dtype = input_dtype
