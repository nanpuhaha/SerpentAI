    ...
    if isinstance(data, dict):
        ...
        if len(data) != len(names):
            if len(data) > 0 and hasattr(data[0], 'shape'):
                raise ValueError(
                    f'Error when checking {exception_prefix}: the list of Numpy arrays that you are passing to your model is not the size the model expected. Expected to see {len(names)} arrays but instead got the following list of {len(data)} arrays: {str(data)[:200]}...'
                )
            ...
            if len(names) == 1:
                ...
                raise ValueError(
                    f'Error when checking {exception_prefix}: you are passing a list as input to your model, but the model expects a list of {len(names)} Numpy arrays instead. The list you passed was: {str(data)[:200]}'
                )
        ...
        if not hasattr(data, 'shape'):
            raise TypeError(
                (
                    (
                        (
                            f'Error when checking {exception_prefix}'
    ...
    if shapes:
        for i in range(len(names)):
            ...
            if len(array.shape) != len(shapes[i]):
                raise ValueError(
                    f'Error when checking {exception_prefix}: expected {names[i]} to have {len(shapes[i])} dimensions, but got array with shape {str(array.shape)}'
                )
            ...
            for j, (dim, ref_dim) in enumerate(zip(array.shape, shapes[i])):
                ...
                if ref_dim and ref_dim != dim:
                    raise ValueError(
                        f'Error when checking {exception_prefix}: expected {names[i]} to have shape {str(shapes[i])} but got array with shape {str(array.shape)}'
                    )
        ...
        if len(x_weight) != len(output_names):
            raise ValueError(
                (
                    (
                        (
                            f'Provided `{weight_type}` was a list of {len(x_weight)} elements, but the model has {len(output_names)}'
                            + ' outputs. '
...
def check_loss_and_target_compatibility(targets, losses, output_shapes):
    ...
    for y, loss, shape in zip(targets, losses, output_shapes):
        if loss.__name__ == 'categorical_crossentropy' and y.shape[-1] == 1:
            raise ValueError(
                f'You are passing a target array of shape {str(y.shape)}'
        ...
        if loss.__name__ in key_losses:
            for target_dim, out_dim in zip(y.shape[1:], shape[1:]):
                if out_dim is not None and target_dim != out_dim:
                    raise ValueError(
                        (
                            f'A target array with shape {str(y.shape)} was passed for an output of shape {str(shape)} while using as loss `{loss.__name__}'
                            + '`. '
                            'This loss expects '
...
def standardize_weights(y, sample_weight=None, class_weight=None,
    ...
    if sample_weight is not None:
        ...
        if y.shape[1] > 1:
            ...
            y_classes = y
        return np.asarray([class_weight[cls] for cls in y_classes])
    else:
...
class GeneratorEnqueuer(object):
    ...
    def stop(self, timeout=None):
        ...
        if self._pickle_safe and self.queue is not None:
            self.queue.close()
    ...
    def compile(self, optimizer, loss, metrics=None, loss_weights=None,
        ...
        if isinstance(sample_weight_mode, dict):
            ...
            for name in self.output_names:
                ...
                if sample_weight_mode.get(name) == 'temporal':
                    weight = K.placeholder(ndim=2, name=f'{name}_sample_weights')
                    sample_weight_modes.append('temporal')
                else:
                    weight = K.placeholder(ndim=1, name=f'{name}_sample_weights')
            ...
            if sample_weight_mode == 'temporal':
                sample_weights = [
                    K.placeholder(ndim=2, name=f'{name}_sample_weights')
                    for name in self.output_names
                ]
                ...
                sample_weights = [
                    K.placeholder(ndim=1, name=f'{name}_sample_weights')
                    for name in self.output_names
                ]
        ...
        for i in range(len(self.outputs)):
            ...
            if len(self.outputs) > 1:
                ...
                self.metrics_names.append(f'{self.output_names[i]}_loss')
        ...
        def append_metric(layer_num, metric_name, metric_tensor):
            ...
            if len(self.output_names) > 1:
                metric_name = f'{self.output_layers[layer_num].name}_{metric_name}'
        ...
        if trainable_weights:
            if K.backend() == 'theano':
                trainable_weights.sort(key=lambda x: x.name or x.auto_name)
            else:
    ...
    def _fit_loop(self, f, ins, out_labels=None, batch_size=32,
        ...
        for epoch in range(initial_epoch, nb_epoch):
            ...
            for batch_index, (batch_start, batch_end) in enumerate(batches):
                ...
                if batch_index == len(batches) - 1 and do_validation:
    ...
    def _test_loop(self, f, ins, batch_size=32, verbose=0):
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batches):
            ...
            if isinstance(batch_outs, list):
                if batch_index == 0:
                    outs.extend(0. for _ in enumerate(batch_outs))
                for i, batch_out in enumerate(batch_outs):
