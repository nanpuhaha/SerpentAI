...
def optimizer_from_config(config, custom_objects=None):
    ...
    elif class_name.lower() in all_classes:
...
class Optimizer(object):
    ...
    def set_weights(self, weights):
        ...
        for pv, p, w in zip(param_values, params, weights):
            if pv.shape != w.shape:
                raise ValueError(
                    f'Optimizer weight shape {str(pv.shape)} not compatible with provided weight shape {str(w.shape)}'
                )
...
def get(identifier, kwargs=None):
    if K.backend() == 'tensorflow' and isinstance(
        identifier, tf.compat.v1.train.Optimizer
    ):
