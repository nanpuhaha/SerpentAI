...
class PReLU(Layer):
    ...
    def __init__(self, init='zero', weights=None, shared_axes=None, **kwargs):
        ...
        self.shared_axes = (
            list(shared_axes)
            if isinstance(shared_axes, (list, tuple))
            else [shared_axes]
        )
    ...
    def build(self, input_shape):
        ...
        self.alphas = self.init(param_shape, name=f'{self.name}_alphas')
...
class ParametricSoftplus(Layer):
    ...
    def build(self, input_shape):
        ...
        self.alphas = K.variable(
            self.alpha_init * np.ones(param_shape), name=f'{self.name}_alphas'
        )

        self.betas = K.variable(
            self.beta_init * np.ones(param_shape), name=f'{self.name}_betas'
        )
...
class SReLU(Layer):
    ...
    def build(self, input_shape):
        ...
        self.t_left = t_left_init(param_shape, name=f'{self.name}_t_left')
        self.a_left = a_left_init(param_shape, name=f'{self.name}_a_left')
        self.t_right = t_right_init(param_shape, name=f'{self.name}_t_right')
        self.a_right = a_right_init(param_shape, name=f'{self.name}_a_right')
