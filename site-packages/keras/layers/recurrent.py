...
class Recurrent(Layer):
    ...
    def call(self, x, mask=None):
        ...
        if self.stateful:
            updates = [(self.states[i], states[i]) for i in range(len(states))]
...
class SimpleRNN(Recurrent):
    ...
    def step(self, x, states):
        ...
        h = x if self.consume_less == 'cpu' else K.dot(x * B_W, self.W) + self.b
...
class GRU(Recurrent):
    ...
    def build(self, input_shape):
        ...
        if self.consume_less == 'gpu':
            self.W = self.add_weight(
                (self.input_dim, 3 * self.output_dim),
                initializer=self.init,
                name=f'{self.name}_W',
                regularizer=self.W_regularizer,
            )
            ...
            self.U = self.add_weight(
                (self.output_dim, 3 * self.output_dim),
                initializer=self.inner_init,
                name=f'{self.name}_U',
                regularizer=self.U_regularizer,
            )
...
class LSTM(Recurrent):
    ...
    def preprocess_input(self, x):
        if self.consume_less != 'cpu':
            return x
        dropout = self.dropout_W if 0 < self.dropout_W < 1 else 0
        input_shape = K.int_shape(x)
        input_dim = input_shape[2]
        timesteps = input_shape[1]
