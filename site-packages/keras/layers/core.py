...
class Lambda(Layer):
    ...
    def get_output_shape_for(self, input_shape):
        if self._output_shape is None:
            ...
            warnings.warn(
                f'`output_shape` argument not specified for layer {self.name} and cannot be automatically inferred with the Theano backend. Defaulting to output shape `{input_shape}` (same as input shape). If the expected output shape is different, specify it via the `output_shape` argument.'
            )
...
class MaxoutDense(Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight(
            (self.nb_feature, input_dim, self.output_dim),
            initializer=self.init,
            name=f'{self.name}_W',
            regularizer=self.W_regularizer,
            constraint=self.W_constraint,
...
class Highway(Layer):
    ...
    def build(self, input_shape):
        ...
        if self.bias:
            ...
            self.b_carry = self.add_weight(
                (input_dim,), initializer='one', name=f'{self.name}_b_carry'
            )
    ...
    def call(self, x, mask=None):
        ...
        return act + (1 - transform_weight) * x
        ...
        if not input_length:
            if hasattr(K, 'int_shape'):
                ...
                if not input_length:
                    raise ValueError(
                        (
                            f'Layer {self.name}' + ' requires to know the length '
