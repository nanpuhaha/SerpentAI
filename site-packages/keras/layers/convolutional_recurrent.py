...
class ConvRecurrent2D(Layer):
    ...
    def get_initial_states(self, X):
        ...
        return [initial_state for _ in range(2)]
    ...
    def call(self, x, mask=None):
        ...
        initial_states = self.states if self.stateful else self.get_initial_states(x)
        ...
        if self.stateful:
            self.updates = []
            self.updates.extend((self.states[i], states[i]) for i in range(len(states)))
...
class ConvLSTM2D(ConvRecurrent2D):
    ...
    def build(self, input_shape):
        ...
        self.W_i = self.init(self.W_shape, name=f'{self.name}_W_i')
        self.U_i = self.inner_init(self.W_shape1, name=f'{self.name}_U_i')
        self.b_i = K.zeros((self.nb_filter,), name=f'{self.name}_b_i')

        self.W_f = self.init(self.W_shape, name=f'{self.name}_W_f')
        self.U_f = self.inner_init(self.W_shape1, name=f'{self.name}_U_f')
        self.b_f = self.forget_bias_init((self.nb_filter,), name=f'{self.name}_b_f')

        self.W_c = self.init(self.W_shape, name=f'{self.name}_W_c')
        self.U_c = self.inner_init(self.W_shape1, name=f'{self.name}_U_c')
        self.b_c = K.zeros((self.nb_filter,), name=f'{self.name}_b_c')

        self.W_o = self.init(self.W_shape, name=f'{self.name}_W_o')
        self.U_o = self.inner_init(self.W_shape1, name=f'{self.name}_U_o')
        self.b_o = K.zeros((self.nb_filter,), name=f'{self.name}_b_o')
    ...
    def conv_step_hidden(self, x, W, border_mode='valid'):
        ...
        return K.conv2d(
            ...
            border_mode=border_mode,
            dim_ordering=self.dim_ordering,
            image_shape=(input_shape[0], out_row, out_col, out_filter),
