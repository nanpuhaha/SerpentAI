...
class RemoteMonitor(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        try:
            ...
            warnings.warn(
                f'Warning: could not reach RemoteMonitor root server at {str(self.root)}'
            )
...
class TensorBoard(Callback):
    ...
    def set_model(self, model):
        ...
        if self.histogram_freq and self.merged is None:
            for layer in self.model.layers:
                ...
                if hasattr(layer, 'output'):
                    if hasattr(tf, 'histogram_summary'):
                        tf.histogram_summary(f'{layer.name}_out', layer.output)
                    else:
                        tf.summary.histogram(f'{layer.name}_out', layer.output)
        ...
        if self.write_graph:
            if hasattr(tf, 'summary') and hasattr(tf.summary, 'FileWriter'):
                ...
                self.writer = tf.train.SummaryWriter(self.log_dir,
                                                     self.sess.graph_def)
        elif hasattr(tf, 'summary') and hasattr(tf.summary, 'FileWriter'):
            self.writer = tf.summary.FileWriter(self.log_dir)
        else:
            self.writer = tf.train.SummaryWriter(self.log_dir)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if (
            ...
            and epoch % self.histogram_freq == 0
        ):
...
class ReduceLROnPlateau(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if current is None:
            warnings.warn(
                f'Learning Rate Plateau Reducing requires {self.monitor} available!',
                RuntimeWarning,
            )
