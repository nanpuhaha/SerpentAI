...
def is_explicit_shape(shape):
    if hasattr(shape, '__iter__'):
        return not any(x is not None and not isinstance(x, int) for x in shape)
    return False
...
def mean(x, axis=None, keepdims=False):
    ...
    """
    dtype = floatx() if 'int' in x.dtype or x.dtype == 'bool' else None
...
def _old_normalize_batch_in_training(x, gamma, beta,
    ...
    if (
        use_cudnn := ndim(x) < 5
        and reduction_axes == [0, 2, 3]
        and (dev.startswith('cuda') or dev.startswith('gpu'))
    ):
...
def concatenate(tensors, axis=-1):
    if not py_all([is_sparse(x) for x in tensors]):
...
def expand_dims(x, dim=-1):
    ...
    """
    # TODO: `keras_shape` inference.
    pattern = list(range(x.type.ndim))
    if dim < 0:
        dim = 0 if x.type.ndim == 0 else dim % x.type.ndim + 1
...
def _preprocess_conv2d_image_shape(dim_ordering, image_shape):
    ...
    if dim_ordering == 'tf' and image_shape:
...
def _preprocess_conv3d_volume_shape(dim_ordering, volume_shape):
    ...
    if dim_ordering == 'tf' and volume_shape:
...
def _preprocess_conv2d_filter_shape(dim_ordering, filter_shape):
    ...
    if dim_ordering == 'tf' and filter_shape:
...
def conv2d(x, kernel, strides=(1, 1), border_mode='valid',
    ...
    elif 'filter_dilation' in inspect.getargspec(T.nnet.conv2d).args:
        conv_out = T.nnet.conv2d(x, kernel,
                                 border_mode=th_border_mode,
                                 subsample=strides,
                                 input_shape=image_shape,
...
def deconv2d(x, kernel, output_shape, strides=(1, 1),
    ...
    if dim_ordering == 'default':
        dim_ordering = image_dim_ordering()
    if dim_ordering not in {'th', 'tf'}:
        raise ValueError(f'Unknown dim_ordering {dim_ordering}')
...
def ctc_batch_cost(y_true, y_pred, input_length, label_length):
    ...
    def ctc_step(y_true_step, y_pred_step, input_length_step, label_length_step):
        y_pred_step = y_pred_step[:input_length_step[0]]
        y_true_step = y_true_step[:label_length_step[0]]
